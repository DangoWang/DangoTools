...
class OctPlayBlastMain(oct_playblast_win.OctPlayBlastWin):
	def __init__(self,parent=None):
		...
		self.proj_comboBox.currentTextChanged.connect(self.proj_dep_comboBox_value_changed)
		self.dep_comboBox.currentTextChanged.connect(self.proj_dep_comboBox_value_changed)
	...
	def draw_hud_text(self, input_path):
		...
		playBlastCMD.draw_all_text_cmd(input_pic=input_path, output_pic=input_path.replace(".png", ".jpg"),
										...
										board_display=board_display, board_color=board_color, board_opacity=board_opacity)
	...
	def on_board_color_pushButton_clicked(self):
		...
		if board_color.isValid():
			self.board_color_pushButton.setStyleSheet(
				'background-color: rgb({}, {}, {});'.format(board_color.getRgb()[0], board_color.getRgb()[1],board_color.getRgb()[2]))
	...
	def add_hud_info(self):
		...
		if label == "<< Artist":
			...
			self.hud_value_add_lineEdit.setText(str(int(cmds.currentTime(q=True)))+"/"+str(listMayaInfo.get_timeslider_time()[2]))
		elif label == "<< FocalLength":
			self.hud_label_add_lineEdit.setText("FocalLength:")
			self.hud_value_add_lineEdit.setText(str(listMayaInfo.get_focal_length(camera_name=self.cam_name_lineEdit.text())))
		elif label == "<< Date":
